...
__global__ void kernel_atomics(int *count_vec, int *input_vec, int num_rows, int num_cols){
   ...
   if(gid < num_rows){
     for(int i = 0; i< num_cols;i++){
		//int writing_pos = input_vec[gid + i * num_rows];
		int writing_pos = input_vec[i + gid * num_cols];
		int istat = atomicAdd(&count_vec[writing_pos], 1); //That simple...
     }
   }
...
kernel_atomics<<<dimGrid, dimBlock>>>(d_subject_prof_count, d_prof_mat, professor_size, subject_per_professor);
