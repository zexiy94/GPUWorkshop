//TO RUN: nvcc -arch=sm_37 -run matrix_transpose_exercise.cu cuda_helper.cu
...
__global__ void gpu_matrix_transpose(float *mat_out,
                                           float *mat_in,
                                           int nrows,
                                           int ncols)
{
...
cudaTick(&ck);
gpu_matrix_transpose<<<dimGrid, dimBlock>>>(d_mat_out, d_mat_in, nrows, ncols);
