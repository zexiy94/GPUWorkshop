                               ...
                               int nrows, int ncols)
{
  ...
  if(col_id < ncols &&  row_id < nrows)
        result_mat[col_id + ncols * row_id] = mat_a[col_id + ncols * row_id] + mat_b[col_id + ncols * row_id];
...
cudaTick(&ck);
gpu_matrix_add<<<dimGrid, dimBlock>>>(d_mat_out, d_mat_a, d_mat_b, nrows, ncols);
