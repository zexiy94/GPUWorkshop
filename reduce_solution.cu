...
float thrust_exclusive_prefix_sum_scan(float *d_input_reduce, int vector_size)
{
   thrust::plus<int> binary_op_plus;
...
cudaTick(&ck);
thrust_exclusive_prefix_sum_scan(d_prefix, d_count, vec_size);
...
cudaTick(&ck2);
float gpu_sum = thrust_exclusive_prefix_sum_scan(d_vec_in, vec_size);
