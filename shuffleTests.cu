...
__global__ void shuffle_tests(float* professor_cost, int* turmas_course, int* professor_turmas, int professor_size, int maxturma_size){
	...
	if(professorid < professor_size){
		...
		if(calculateFlag == 1) int istat = atomicAdd(&professor_cost[professorid],1.0f); 
	}
}
...
int main(int argc, char** argv){
 ...
 h_professor_cost = (float*)malloc(professor_size*sizeof(float)); 
  ...
  printf("dimBlock: %d \n", shuffleBlock.x);
   ...
   shuffle_tests<<<shuffleGrid, shuffleBlock>>>(d_professor_cost, d_turmas_course, d_professor_turmas, professor_size, maxturma_size);
   ...
   printf("criteria total cost: %f, against naive cost: %f \n", totresult, professor_size*maxturma_size*1.0f);
