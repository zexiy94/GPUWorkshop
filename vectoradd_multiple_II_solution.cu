...
#include <stdio.h>
__constant__ float const_vector_b[1024];

__global__ void kernel_vector_multi_add_constant(float *result_vec, float *vec_a, int vector_size, int number_of_vectors){
  ////// YOUR CODE GOES HERE!////////////////////////////////////////////
  ...
  if(gid >= vector_size * number_of_vectors) return;
	...
	result_vec[gid] = vec_a[gid] + const_vector_b[local_id];
}
...
CudaSafeCall(cudaMalloc((void**)&d_vec_out, vec_multi_size_bytes));
//.... ALLOCATE THE REST OF THE VECTORS
CudaSafeCall(cudaMalloc((void**)&d_vec_a, vec_multi_size_bytes));
CudaSafeCall(cudaMalloc((void**)&d_vec_b, vec_size_bytes));


//------ Step 2: Copy Memory to the device-------
printf("Transfering data to the Device..\n");
CudaSafeCall(cudaMemcpy(d_vec_a, h_vec_a, vec_multi_size_bytes, cudaMemcpyHostToDevice));
CudaSafeCall(cudaMemcpy(d_vec_b, h_vec_b, vec_size_bytes, cudaMemcpyHostToDevice));
CudaSafeCall(cudaMemcpyToSymbol(const_vector_b, h_vec_b, vec_size_bytes));
...
cudaTick(&ck2);
kernel_vector_multi_add_constant<<<dimGrid, dimBlock>>>(d_vec_out, d_vec_a, vec_size, number_of_vectors);
cudaTock(&ck2, "kernel_vector_multi_add_constant");
...
CudaSafeCall(cudaMemcpy(h_vec_out_gpu, d_vec_out, vec_multi_size_bytes, cudaMemcpyDeviceToHost));
...
std::cout << "constant version gpu is " << cpuck.elapsedMicroseconds/ck2.elapsedMicroseconds << " times faster" << std::endl;
...
CudaSafeCall(cudaFree(d_vec_out));
CudaSafeCall(cudaFree(d_vec_a));
CudaSafeCall(cudaFree(d_vec_b));
